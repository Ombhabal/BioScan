...
export function GeneSequence({
  ...
  /** ✅ Update slider values when geneBounds or positions change */
  useEffect(() => {
    ...
    setSliderValues((prev) => {
      ...
      if (prev.start === newValues.start && prev.end === newValues.end)
        return prev;
      return newValues;
    });
      ...
      if (isDraggingStart) {
        if (!isNaN(currentEndNum)) {
          if (currentEndNum - newPosition + 1 > maxViewRange) {
            if (String(currentEndNum - maxViewRange + 1) !== startPosition) {
              onStartPositionChange(String(currentEndNum - maxViewRange + 1));
            }
          } else if (
            newPosition < currentEndNum &&
        ...
        if (!isNaN(currentStartNum)) {
          if (newPosition - currentStartNum + 1 > maxViewRange) {
            if (String(currentStartNum + maxViewRange - 1) !== endPosition) {
              onEndPositionChange(String(currentStartNum + maxViewRange - 1));
            }
          } else if (
            newPosition > currentStartNum &&
            String(newPosition) !== endPosition
          ) {
        ...
        if (String(newStart) !== startPosition)
          onStartPositionChange(String(newStart));
        if (String(newEnd) !== endPosition) onEndPositionChange(String(newEnd));
      }
    };
